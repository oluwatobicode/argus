...
const MAX_FILE_SIZE = 1000000; /* a 1MB+ "source file" is a bundle — skip */
...
const clip = (line) => line.slice(0, MAX_LINE_LENGTH);
/* mutates the frames in place — called between parseStack and buildEnvelope */
export function attachSourceContext(frames) {
    let attached = 0;
    for (const frame of frames) {
        if (attached >= MAX_FRAMES_WITH_CONTEXT)
            return;
        try {
            if (!isAppFrame(frame.filename))
                continue;
            ...
            if (!lines || frame.lineno < 1 || frame.lineno > lines.length)
                continue;
