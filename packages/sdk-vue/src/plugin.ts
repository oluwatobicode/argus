...
/*
 * Vue exposes the resolved component definition on the internal `$` handle,
 * and the SFC compiler stamps `__name` on it from the filename. Neither is in
 * Vue's public types, so read them through a narrow shape rather than `any`.
 */
...
function componentName(
  ...
  const internals = instance as unknown as InstanceInternals;
  return (
 ...
 */
export function attachVueErrorHandler(app: App): void {
